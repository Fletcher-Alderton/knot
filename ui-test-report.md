...
| Smart “＋ Open board” action | Fixed (automated) | One native directory picker now opens directories containing an existing board and initializes `board.md` plus `cards/` when no board exists. |
...
| Board tabs and switching boards | Pass | Created two disposable boards, saw both tabs, and switched back to the first board with its two cards intact. |
...
| Rename board | Fixed (automated) | Rename uses an accessible in-app form and persists through the existing backend command. |
| Rename column | Fixed (automated) | Column rename uses a prefilled accessible in-app form and persists through the existing backend command. |
| Add column | Fixed (automated) | Add column uses an accessible in-app form and persists through the existing backend command. |
