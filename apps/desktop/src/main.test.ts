...
it('pairs a serialized endpoint address for the current board',async()=>{
 ...
 it('registers watcher before loading cards to avoid missing cloud edits',async()=>{
   const invoke=vi.fn(async(command:string)=>{if(command==='open_default_board')return {path:'/cloud-board',board_id:'cloud',title:'Cloud',columns:[{id:'todo',name:'Todo'}],cards:[]};if(command==='watch_board')return true;if(command==='list_cards')return [{id:'ready',title:'Ready',body:'',column:'todo',labels:[]}];if(command==='sync_status')return {connected:false,trusted_peers:0,connection:'offline'};if(command==='endpoint_info')return {endpoint_id:'local',address:'local'};if(command==='list_trusted_peers')return [];throw new Error(command)});
   setInvokeForTests(invoke as any);await openDefaultBoard();
   expect(state.cards[0]?.id).toBe('ready');expect(state.loading).toBe(false);
   const commands=invoke.mock.calls.map(([command])=>command);expect(commands.indexOf('watch_board')).toBeLessThan(commands.indexOf('list_cards'));
   setInvokeForTests(null);
 });
