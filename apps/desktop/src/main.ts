...
export interface AppCapabilities { local_ai:boolean; mobile:boolean }
export const capabilities:AppCapabilities={local_ai:false,mobile:false};
...
export async function loadCapabilities(){try{const value=await invoke<Partial<AppCapabilities>>('app_capabilities');capabilities.local_ai=value.local_ai===true;capabilities.mobile=value.mobile===true;capabilitiesLoaded=true;state.error='';}catch(e){capabilitiesLoaded=false;state.error='Unable to determine app capabilities: '+message(e);}render();}
...
export async function loadModels(){
  ...
  try{
    ...
    state.localModels=capabilities.local_ai?await invoke<LocalModel[]>('list_local_models'):[];
    if(state.modelSettings.provider==='ollama')state.ollamaModels=await invoke<{name:string;size_bytes:number;modified_at?:string}[]>('list_ollama_models',{url:state.modelSettings.ollama_url});
    else if(state.modelSettings.provider==='openai'){if(state.modelSettings.openai_api_key)await loadOpenAIModels();}
    else if(capabilities.local_ai&&state.modelSettings.keep_model_loaded&&state.modelSettings.model_id)await invoke('load_local_model',{id:state.modelSettings.model_id});
    else if(!capabilities.local_ai&&state.modelSettings.provider==='huggingface')state.modelFeedback='Local AI is disabled in this build. Choose Ollama or an OpenAI-compatible provider.';
    render();
...
async function openQuickAdd(){if(!state.boardPath){state.error='Open a board before using Quick Add.';render();return;}if(!capabilitiesLoaded)await loadCapabilities();if(!capabilitiesLoaded){state.error='Unable to determine AI capabilities. Try again.';render();return;}const provider=state.modelSettings.provider;if(provider==='huggingface'&&!capabilities.local_ai){state.view='settings';state.error='Local AI is disabled. Choose Ollama or an OpenAI-compatible provider in Settings.';render();return;}if(!provider||!state.modelSettings.model_id){state.view='settings';state.error='Choose an AI provider and model in Settings before using Quick Add.';render();return;}state.quickAddOpen=true;render();queueMicrotask(()=>document.querySelector<HTMLInputElement>('#quick-add-input')?.focus());}
...
function modelSettingsSection(){
  ...
  const usingOllama=state.modelSettings.provider==='ollama'||(!capabilities.local_ai&&!usingOpenAI);
