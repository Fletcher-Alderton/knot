...
export async function loadArchivedCards(){if(!state.boardPath)return;state.archivedLoading=true;render();try{const result=await invoke<ArchivedCard[]>('list_archived_cards',{path:state.boardPath});state.archivedCards=(result||[]).map(item=>({card:normalize(item.card),revisionId:item.revision_id,archivedAt:item.archived_at}));state.error='';}catch(e){state.error='Unable to load archived cards: '+message(e);}finally{state.archivedLoading=false;render();}}
...
function prettyEditorDate(value:string|undefined):string{
  ...
  return Number.isNaN(d.getTime())?'':new Intl.DateTimeFormat('en',{month:'short',day:'numeric'}).format(d);
}
...
function settingsPage(){return `<section class="workspace settings" data-page="settings"><div class="settings-header"><span class="eyebrow">IROHMD</span><h1>Settings</h1></div><div class="settings-layout">${modelSettingsSection()}<div class="board-settings">${labelsSettingsSection()}${syncSettingsSection()}</div></div></section>`;}
...
function bind(){
  ...
  const columns=document.querySelector<HTMLElement>('.workspace[data-page="board"] .columns');
  if(state.showArchived&&columns){columns.insertAdjacentHTML('afterend',archivedSection());document.querySelectorAll<HTMLElement>('[data-restore-archived]').forEach(button=>button.addEventListener('click',()=>{const item=state.archivedCards.find(card=>card.revisionId===button.dataset.restoreArchived);if(item)void restoreArchivedCard(item);}));}
